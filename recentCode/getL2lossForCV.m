...
for fisherScoringIteration = 1:maxIteration
    ...
    for jqz = 2+(-depth:depth)
      ...
      %largeInputObservedInformation = (xis(large)'.^2).* ...
      %  (-1 + (firstRatio./xisNus(large)) + firstRatio.^2)*2;
...
%      smallInputScores = xis(small)' .* ratioForSmalls * w;
      informationSmalls = w*(-1 + xis(small)'.^2 - (gammaSmalls ./ nuHat(small)) - gammaSmalls.^2 );
%smallInputObservedInformation = (xis(small)'.^2).* ...
%          (-1 + (smallFirstRatio./xisNus(small)) + ratioForSmalls.^2)*2;
