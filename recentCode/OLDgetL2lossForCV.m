...
%3.a - alternatively, do inclusion exclusion for LOOCV.
%mask = (mask == 255);
%numberOfUsedVoxels = sum(sum(sum(mask)));
%numberOfUsedDoxels = 515*numberOfUsedDoxels;
%testedVoxels = zeros(numberOfUsedDoxels,1);
%for i = 1:515
%    sigma = sigmas(i);
%    qx = intBvecs(i, 1);
%    qy = intBvecs(i, 2);
%    qz = intBvecx(i, 3);
%    
%    voxelLocations = ind2sub(matrixSize, 1); %Effff...
...
for fisherScoringIteration = 1:maxIteration
    ...
    for jqz = 2+(-depth:depth)
      ...
      largeInputScores = xis(greaterThanThreshold)'.*firstRatio*w;  % I thought the score was different...
                                    ...
                                    %  the - lambda..
      if any(xisNus == 0)
          error('You are about to divide by zero due to xi*nu = 0 while computing observed information.');
      end
      
      largeInputObservedInformation = (xis(greaterThanThreshold)'.^2).* ...
        (-1 + (firstRatio./xisNus(greaterThanThreshold)) + firstRatio.^2)*2;
      if(any(isnan(largeInputObservedInformation)))
          error('Hit NAs on large input observed information.');
      end
      ...
      smallInputObservedInformation = (xis(smallerThanThreshold)'.^2).* ...
          (-1 + (smallFirstRatio./xisNus(smallerThanThreshold)) + smallFirstRatio.^2)*2;
      if (any(isnan(smallInputObservedInformation)))
          error('Hit NAs on small input observed information.');
      end
