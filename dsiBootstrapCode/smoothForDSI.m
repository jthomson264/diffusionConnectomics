function [ newDWI ] = smoothForDSI( DWI, btable )
...
imageX = size(DWI.img, 1);
imageY = size(DWI.img, 2);
imageZ = size(DWI.img, 3);
imageQ = size(DWI.img, 4);
...
for ix = 1:imageX
    for iy = 1:imageY
        for iz = 1:imageZ
            ...
            currentVoxelImage = squeeze(DWI.img(ix,iy,iz,:));
            DWI.img(ix,iy,iz,:) = BlurMatrix * double(currentVoxelImage);
        end
    end
end


%write_nii('DSItruth.nii');
newDWI = DWI;
end
