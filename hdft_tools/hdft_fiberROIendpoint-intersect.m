function [any_intx, output]= hdft_fiberROIintersect(track_file, roi_coords_object, varargin)
...
output.beg_xyz = fiber_xyz.beg_xyz(beg_intx,:);
output.end_xyz = fiber_xyz.beg_xyz(end_intx,:);

% Convert to voxel coordinates too
[vx vy vz] = mm2voxel(output.beg_xyz(:,1),output.beg_xyz(:,2),output.beg_xyz(:,3),trk.header);
output.beg_v_xyz = [vx vy vz];

[vx vy vz] = mm2voxel(output.end_xyz(:,1),output.end_xyz(:,2),output.end_xyz(:,3),trk.header);
output.end_v_xyz = [vx vy vz];

% Grab all indes locations next
any_intx = unique([find(beg_indx); find(end_indx)]);
