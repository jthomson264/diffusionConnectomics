% This is going to create a NII view for ....  not a lot of pixels....
function [ smoothedData ] = smoothPanesOfNII()
...
for qIndex = 1:4
    ...
    for vy = (1 + N_SPATIAL_WINDOW):(96 - N_SPATIAL_WINDOW)
        for vz = (1 + N_SPATIAL_WINDOW):(50 - N_SPATIAL_WINDOW)
            ...
            nuVector = ...
                solveForNuWithWeights(allImageValuesForVoxel, sigmas.^2, fullSimilarities, 30);
            smoothedImageSample(vx, vy, vz, vq) = nuVector(30);
            % Do ish.
        end
    end
    ...
    % Do yPane
    vy = yPane;
    for vx = (1 + N_SPATIAL_WINDOW):(96 - N_SPATIAL_WINDOW)
        for vz = (1 + N_SPATIAL_WINDOW):(50 - N_SPATIAL_WINDOW)
            ...
            allImageValuesForVoxel = double(data.img(vxRange, vyRange, vzRange, :));
            allImageValuesForVoxel = reshape(allImageValuesForVoxel, windowWidth^3, 515);
    ...
    disp('... completed a saggital plane ...');
    ...
    disp('... completed a transversal plan ...');
    
    
    disp('Completed three full panes for one q-vector...');
