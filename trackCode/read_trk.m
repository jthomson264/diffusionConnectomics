...
if tracks.header.n_scalars == 0
    ...
    nfloat = 3 + tracks.header.n_scalars;
    if tracks.header.n_properties == 0
        for i=1:no_fibers
            tracks.fiber{i}.num_points = fread(fid,1,'int');
            dummy = fread(fid,[nfloat,tracks.fiber{i}.num_points],'float')';
