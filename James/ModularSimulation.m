...
for i=2:length(qvecs(:,1))
    ...
    NuCalc(i,1)=S0*exp(-(qvecs(i,:)*Xc*qvecs(i,:)'));
        ...
        %for non-DTI
    %NuCalc(i,1) = exp(-bvalues(i,1)*ADC) %Might need to divide by 1000
    %here!
end
...
tempGrad(1) = 0;
%for Rician
tempGrad = -( -NuCalc./sigma2s + besseli(1, Xs.*NuCalc./sigma2s)/besseli(0,Xs.*NuCalc./sigma2s)*Xs./sigma2s ).* NuCalc ;
