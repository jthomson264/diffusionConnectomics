function [ smoothedImageSample ] = smoothNII(sampleRange, data, btab, spatialSigma, frequencySigma, bscaling)
...
%  Spatial Range is just for testing purposes, so I can get small samples
%  of what the smoothing looks like w/out running it on the full brain.
%  It will look like:
%            .xRange = [21, 34]
%            .yRange = [30, 46]
%            .zRange = [38]
%            .qRange = [30, 150, 350]     Theses are qvec indices.

nX = size(sampleRange.xRange, 2);
nY = size(sampleRange.yRange, 2);
nZ = size(sampleRange.zRange, 2);
nQ = size(sampleRange.qRange, 2);
smoothedImageSample = zeros(nX, nY, nZ, nQ);
...
qindex = 1;
for vq = sampleRange.qRange
    xindex = 1;
    for vx = sampleRange.xRange
        yindex = 1;
        for vy = sampleRange.yRange
            zindex = 1;
            for vz = sampleRange.zRange
